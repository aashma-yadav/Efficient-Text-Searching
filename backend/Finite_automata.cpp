...
// ONCE on startup. The transition table (TF) is inherently pattern-dependent
// so it's rebuilt per query, same as the algorithm's true complexity -- but
// disk I/O and process-spawn overhead only happen once.
//
// Usage: ./fa_bin <textfile> [--once]
// ---------------------------------------------------------------------------
int main(int argc, char **argv)
{
	...
	if (once)
	{
		...
		getline(cin, s2);
		search(s2, s1);
		cout << "Number of Occurences: " << count_fa << endl;
		return 0;
	}
